...
class PlayingCharacter(Character):
    ...
    def update(self, time_passed):
        ...
        if (
            ...
            or cblocals.global_lastMouseRightClickPosition
        ):
            self.stopThinking()
        ...
        if cblocals.global_lastMouseLeftClickPosition:
            self.navPoint.set(
                self.currentLevel.transformToLevelCoordinate(
                    cblocals.global_lastMouseLeftClickPosition
                )
            )
            cblocals.global_lastMouseLeftClickPosition = ()
        if cblocals.global_lastMouseRightClickPosition and not self.isAttacking():
            attackHeading = Vector2.from_points(
                self.position,
                self.currentLevel.transformToLevelCoordinate(
    ...
    seeking = property(
        lambda self: self._trackedEnemy,
        _setSeeking,
        doc="""The enemy that the hero is hunting""",
    )
