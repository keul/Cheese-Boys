...
class Stealth(object):
    ...
    def _setStealth(self, value):
        if value == False:
            logging.info(
                "%s can't go in stealth mode again for %s millisec"
                % (self, self.stealthRestTimeNeeded)
            )
            self.last_stealth_timing = cblocals.game_time
        self._stealth = value

    stealth = property(
        lambda self: self._stealth,
        _setStealth,
        doc="""The stealth status (if the character is trying to hide himself or not)""",
    )
    ...
    def stealthIndex(self):
        ...
        index = base_index * level_stealth - ((self.stealthLevel - 1) * 0.05)
        if index < 0.2:
            index = 0.2
        return index
    ...
    def canStealthAgain(self):
        ...
        """
        return (
            cblocals.game_time - self.last_stealth_timing > self.stealthRestTimeNeeded
        )
    ...
    def getTimingBeforeCheckForStealth(self):
        ...
        return int(base_timing - base_timing * (stealthLevel * 0.05))
    ...
    def getPreySightRangeReductionFactor(self):
        ...
        reduction = 1.0
        if self.stealth:
            reduction -= 0.1 + (0.05 * self.stealthLevel)
        return reduction
    ...
    def adjustStealthIndexFromPosition(cls, rogueStealthIndex, rogue, prey):
        ...
        if distance < preySight / 5:
            modifier += 0.2  # malus 20%
        elif distance < preySight / 3:
            modifier += 0.15  # malus 15%
        elif distance < preySight / 3 * 2:
            modifier += 0.10  # malus 10%
        ...
        logging.debug(
            "Prey heading: %s - Prey to rogue: %s - Angle: %s"
            % (prey.heading, prey_to_rogue, angle_between)
        )
        ...
        if rogueStealthIndex > 0.95:
            rogueStealthIndex = 0.95
        elif rogueStealthIndex < 0.2:
            rogueStealthIndex = 0.2
    ...
    def check_antiStealth(self, enemy):
        ...
        if enemy_uid in stealthEnemies:
        ...
        enemyStealthIndex = self.adjustStealthIndexFromPosition(
            enemyStealthIndex, enemy, self
        )
        ...
        logging.info(
            "%s try to find %s that have a stealth index of %0.2f (base is %0.2f): rolled %0.2f (%s)"
            % (
                self,
                enemy,
                enemyStealthIndex,
        ...
        stealthEnemies[enemy_uid] = {
            "result": result,
            "timing": cblocals.game_time,
        }
        return result
    ...
    def canSeeHiddenCharacter(self, target):
        ...
        if not target.stealth:
            return True
        if target_uid in self._stealthEnemies:
    ...
    def noticeForHiddenCharacter(self, enemy):
        ...
        enemy_uid = enemy.UID()
        self._stealthEnemies[enemy_uid] = {
            "result": True,
            "timing": cblocals.game_time,
        }
