...
class GameLevel(GridMapSupport):
    ...
    def __init__(self, name, size, background=""):
        ...
        speech = GameGroup(
            "speech", updatable=True
        )  # speechs are handled in a special way
    ...
    topleft = property(
        lambda self: self._topleft,
        _setTopLeft,
        doc="""The topleft drawing point inside the level background image""",
    )
    ...
    def addGroup(self, group, zindex=10):
        ...
        if group.updatable:
            ...
            self._groups_toupdate.sort(key=lambda x: x[0])
        if group.drawable:
            self._groups_todraw.append((zindex, group))
            self._groups_todraw.sort(key=lambda x: x[0])
    ...
    def generateRandomPoint(self, fromPoint=(), maxdistance=0):
        ...
        return self.normalizePointOnLevel(
            cbrandom.randint(startX, endX), cbrandom.randint(startY, endY)
        )
    ...
    def normalizePointOnLevel(self, x, y):
        ...
        if x < 1:
            x = 1
        elif x > self.levelSize[0] - 1:
            x = self.levelSize[0] - 1
        if y < 1:
            y = 1
        elif y > self.levelSize[1] - 1:
            y = self.levelSize[1] - 1
        return x, y
            ...
            if character.side != charas.side and distanceFromCharas <= sight:
                ...
                if character.hasFreeSightOn(charas):
                    if charas.stealth:
                        ...
                        if distanceFromCharas <= int(
                            sight * charas.getPreySightRangeReductionFactor()
                        ):
    ...
    def draw(self, screen):
        ...
        if self._background:
            screen.blit(
                self._background.subsurface(
                    pygame.Rect(self.topleft, cblocals.GAME_SCREEN_SIZE)
                ),
                (0, 0),
            )
    ...
    def addPhysicalSightBlockingBackground(
        self, position, size, groups=["physical", "visual_obstacles"]
    ):
        ...
        if self.shadow:
            if not self.presentation:
                if not refSprite.outOfScreen:
                    spriteposx, spriteposy = self.transformToScreenCoordinate(
                        refSprite.position
                    )
                    imgsizew, imgsizeh = cblocals.SHADOW_IMG_SIZE
                    surface.blit(
                        cblocals.shadow_image,
                        (spriteposx - imgsizew / 2, spriteposy - imgsizeh / 2),
                    )
                else:
