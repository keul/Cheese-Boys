...
TIMESPAMP_LINE_REGEXP = (
    r"""^(\s)*(\[\d\d:\d\d:\d\d \d\d\d\]|\[\+\d\d:\d\d:\d\d \d\d\d])(\s)*(#.*)?$"""
)
...
class PresentationParser(object):
    ...
    def _checkVersionLineSyntax(self, line, lnumber):
        ...
        if not re_versionLineCheck.match(line):
            raise CBPParsingException(
                'First data line isn\'t version information; found "%s"' % line, lnumber
            )
    ...
    def _loadData(self):
        ...
        operations.sort(key=timestamp_sort)
    ...
    @classmethod
    def replaceCbpFileAbsoluteTimestamps(
        cls, filename, first_timestamp=None, outFile=None
    ):
        """
        ...
        try:
            ...
            pp = PresentationParser(
                os.path.join(os.getcwd(), filename), presentation_dir=""
            )
            pp.load()
        ...
        for line in pp.text.split("\n"):
            ...
            if (
                not first_timestamp
                and re_timeStampAbsoluteCheck.match(line)
                and timevalue is not None
            ):
                ...
                print(
                    "replacing at line %s: timestamp %s with %s" % (cnt, old_ts, new_ts)
                )
