...
class Attack(object):
    ...
    def _updateRect(self, attackOriginVector):
        ...
        attackEffectCenterVector = (
            character.attackHeading * attackRange / 2 + attackOriginVector
        )
        attackEffectCenterVectorTuple = attackEffectCenterVector.as_tuple()
        self.rect = pygame.Rect(
            (
                attackEffectCenterVectorTuple[0] - attackEffect / 2,
                attackEffectCenterVectorTuple[1] - attackEffect / 2,
            ),
    ...
    def drawPhase1(self, surface, attackOriginVector):
        ...
        if not self._phase1:
            ...
            event = pygame.event.Event(
                cblocals.ATTACK_OCCURRED_EVENT,
                {"character": self._character, "attack": self},
            )
