...
class Rain(object):
    ...
    def update(self, time_passed):
        ...
        for x in range(len(self._raindrops)):
            ...
            rain_heading = (self._rain_heading[0] + self._wind, self._rain_heading[1])
            movementx, movementy = (
                rain_heading[0] * distance,
                rain_heading[1] * distance,
            )
            raindrop["position"] = [
                raindrop["position"][0] + movementx,
                raindrop["position"][1] + movementy,
            ]
    ...
    def changeRainFrequency(self):
        ...
        logging.debug(
            "Rain frequence changed from %s to %s" % (self._raindrops_length, ndrops)
        )
        self._raindrops_length = ndrops
    ...
    def changeWindStrength(self):
        ...
        wind = self._wind
        wind = cbrandom.choice((-0.4, -0.3, -0.2, -0.1, 0))
