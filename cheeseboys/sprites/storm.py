...
def randomTime():
    return cbrandom.uniform(0.2, 0.4)
...
class Thunders(GameSprite):
    ...
    def image(self):
        ...
        if phase == 1:
            ...
            self._image = GameSprite.generateEmptySprite(
                self.rect.size, alpha=245, fillWith=(255, 255, 255)
            )
            return self._image
        if phase == 2:
            self._image = GameSprite.generateEmptySprite(
                self.rect.size, alpha=245, fillWith=(0, 0, 0)
            )
            return self._image
        if phase == 3:
            self._image = GameSprite.generateEmptySprite(
