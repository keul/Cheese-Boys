...
class SpeechCloud(GameSprite):
    ...
    def initSpeech(self):
        ...
        self.addToGameLevel(
            level, firstPosition=level.transformToLevelCoordinate(self.rect.midbottom)
        )
    ...
    text = property(
        lambda self: self._text, _setText, doc="""The text of the SpeechCloud"""
    )
    ...
    def _generateTextLines(cls, text):
        ...
        w = (
            BORDER_PADDING
            + max([speech_font.size(x)[0] for x in textlines])
            + BORDER_PADDING
        )
        ...
        for line in textlines:
            ...
            if w > CLOUD_MAX_WIDTH:
                newtextlines.extend(
                    utils.normalizeTextLength(
                        line, speech_font, CLOUD_MAX_WIDTH - BORDER_PADDING * 2
                    )
                )
            else:
    ...
    def _getRect(self):
        ...
        h = (
            BORDER_PADDING
            + len(textlines) * speech_font_h
            + PER_LINE_PADDING * (len(textlines) - 1)
            + BORDER_PADDING
        )
        character = self._character
        ...
        if rect.topleft[0] < 0:
            rect.left = 0
        elif rect.topright[0] > cblocals.GAME_SCREEN_SIZE[0]:
            rect.right = cblocals.GAME_SCREEN_SIZE[0]
        if rect.topleft[1] < 0:
            rect.top = 0
        elif rect.bottomleft[1] > cblocals.GAME_SCREEN_SIZE[1]:
            rect.bottom = cblocals.GAME_SCREEN_SIZE[1]
        return rect
    ...
    def image(self):
        ...
        for text_line in self._generateTextLines(self.text):
            srf.blit(
                speech_font.render(text_line, True, self.textcolor),
                (BORDER_PADDING, ptop),
            )
