...
from cheeseboys.presentation.presentation_parser import (
    PresentationParser,
    CBPParsingException,
)
...
class TestPresentationParser(CheeseBoysTestCase):
    ...
    def testVersionLine(self):
        ...
        self.assertRaises(
            CBPParsingException, self.pparser._checkVersionLineSyntax, line, 6
        )
        line = self.pparser._lines[3]
        self.assertRaises(
            CBPParsingException, self.pparser._checkVersionLineSyntax, line, 4
        )
        self.assertRaises(
            CBPParsingException,
            self.pparser._checkVersionLineSyntax,
            line.strip() + "   # no comment on version line",
            4,
        )
        self.assertEqual(
            self.pparser._checkVersionLineSyntax(line.strip(), 4), (2, 0, 0)
        )

    def testCheckSyntax(self):
        """Check that application can handle correctly a whole file block"""
        self.assertEqual(self.pparser.checkSyntax(), None)

    def test_getTimeStamp(self):
        """Check loading for timestamps value(s)"""
        self.assertEqual(self.pparser._getTimeStamp("[00:00:00 000]"), "00:00:00 000")
        self.assertEqual(self.pparser._getTimeStamp("[+00:00:00 001]"), "+00:00:00 001")

    def test_loadData(self):
        """Test the loading of a complete commands datablock"""
        self.pparser._loadData()
        self.assertEqual(
            self.pparser.data["operations"][1]["commands"][2], "dummy = 'abc'"
        )
...
class TestPresentation(CheeseBoysTestCase):
    ...
    def test_timestampValueToString(self):
        ...
        self.assertEqual(timestampValueToString(0), "00:00:00 000")
        self.assertEqual(timestampValueToString(123), "00:00:00 123")
        self.assertEqual(timestampValueToString(12 * 1000 + 123), "00:00:12 123")
        self.assertEqual(
            timestampValueToString(30 * 1000 * 60 + 24 * 1000 + 123), "00:30:24 123"
        )
        self.assertEqual(
            timestampValueToString(
                2 * 1000 * 60 * 60 + 30 * 1000 * 60 + 24 * 1000 + 123
            ),
            "02:30:24 123",
        )
    ...
    def test_timestampStringToValue(self):
        ...
        self.assertEqual(timestampStringToValue("00:00:00 000"), 0)
        self.assertEqual(timestampStringToValue("00:00:00 050"), 50)
        self.assertEqual(timestampStringToValue("00:00:02 050"), 2 * 1000 + 50)
        self.assertEqual(
            timestampStringToValue("00:13:02 050"), 13 * 1000 * 60 + 2 * 1000 + 50
        )
        self.assertEqual(
            timestampStringToValue("05:00:02 050"), 5 * 1000 * 60 * 60 + 2 * 1000 + 50
        )
