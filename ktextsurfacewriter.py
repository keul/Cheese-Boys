...
class KTextSurfaceWriter(object):
    ...
    def __init__(self, rect, font=None, color=(0,0,255,0), fillcolor=(0,0,0,0)):
        self.rect = rect
        if not font:
    ...
    @classmethod
    def normalizeTextLength(cls, text_too_long, font, max_length):
        ...
        """
        words = [x for x in text_too_long.split(" ") if x]
        words_removed = []
        tooLong = True
        txt1 = txt2 = ""
        while tooLong:
        ...
        if txt2==text_too_long:
            # I cut the line at the max wide
            while font.size(txt2)[0]>max_length:
                txt2 = txt2[:-1].strip()
        if font.size(txt2)[0]<=max_length:
            return [txt1, txt2]
        else:
            return [txt1] + cls.normalizeTextLength(txt2, font, max_length)
    ...
    def _prepareText(self):
        ...
        for lines in text.split("\n"):
            ...
            if lw>rw:
                newtextlines = self.normalizeTextLength(text, self.font, rw)
            else:
                newtextlines = [text,]
    ...
    def draw(self, surface):
        ...
        resultPage = self._prepareText()
        rect = self.rect
        i = 0
        for line in resultPage:
            ren = self.font.render(self.text, 1, self.color, self.fillcolor)
